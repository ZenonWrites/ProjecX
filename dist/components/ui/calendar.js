...
function Calendar(_a) {
    ...
    return (_jsx(DayPicker, __assign({ showOutsideDays: showOutsideDays, className: cn("p-3", className), classNames: __assign({ months: "flex flex-col sm:flex-row gap-2", month: "flex flex-col gap-4", caption: "flex justify-center pt-1 relative items-center w-full", caption_label: "text-sm font-medium", nav: "flex items-center gap-1", nav_button: cn(buttonVariants({ variant: "outline" }), "size-7 bg-transparent p-0 opacity-50 hover:opacity-100"), nav_button_previous: "absolute left-1", nav_button_next: "absolute right-1", table: "w-full border-collapse space-x-1", head_row: "flex", head_cell: "text-muted-foreground rounded-md w-8 font-normal text-[0.8rem]", row: "flex w-full mt-2", cell: cn("relative p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([aria-selected])]:bg-accent [&:has([aria-selected].day-range-end)]:rounded-r-md", props.mode === "range"
            ...
            IconLeft: function (_a) {
                ...
                return (_jsx(ChevronLeft, __assign({ className: cn("size-4", className) }, props)));
            },
            IconRight: function (_a) {
                var className = _a.className, props = __rest(_a, ["className"]);
                return (_jsx(ChevronRight, __assign({ className: cn("size-4", className) }, props)));
            },
        } }, props)));
}
export { Calendar };
